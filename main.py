...
def get_args():
    ...
    parser.add_argument("-m","--model_name", help="model cnn name", default="model")
    parser.add_argument("-mc","--model_corr", help="model corr name", default="model_corr")
    parser.add_argument("-p","--protein", help="protein",nargs='+', default="RBP1")
    args = parser.parse_args()
    return args.directory, args.model_name, args.model_corr, args.protein
...
def workflow_single_protein_data(PROTEIN_NAME):
    ...
    prediction, features = find_proteins_features(model, shifts_RNAcompete_master_list,PROTEIN_NAME)
    return features
...
def workflow_fit_all_protein_data():
    ...
    for protein in PROTEINS:
        ...
        model_corr_name = get_args()[2]
        model=create_simple_NN_model(features, target, protein, model_corr_name) # creates corr model
        get_final_output_per_protein(features, protein, model_corr_name) # creates pred output file per protein
        get_corr(protein)
